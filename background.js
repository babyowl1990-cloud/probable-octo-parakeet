...
//
// Safety rails:
//  - Only the origin and path in config.js are ever touched.
...
const SITE_ORIGIN = CONFIG.origin;
const SITE_PATH = CONFIG.path;
const SITE_URL = new URL(SITE_ORIGIN);
const SITE_HOST = SITE_URL.hostname;
const PREFIX = "tl_";

// The exact origin AND path prefix. On GitHub Pages every repo of a user shares one origin,
// so the path keeps other repos out.
const isSite = (url) => {
  try {
    const u = new URL(url);
...
async function getSiteCookies() {
  const all = await chrome.cookies.getAll({ url: SITE_ORIGIN + SITE_PATH });
...
async function restore() {
  ...
  for (const c of stash.cookies) {
    ...
    if (c.secure && SITE_URL.protocol === "https:") details.secure = true;
    try {
...
chrome.cookies.onChanged.addListener(({ cookie }) => {
  ...
  if (cookie.domain.replace(/^\./, "") !== SITE_HOST) return;
...
chrome.webNavigation.onBeforeNavigate.addListener(
  ...
  {
    url: [{
      hostEquals: SITE_HOST,
      schemes: [SITE_URL.protocol.replace(":", "")],
      pathPrefix: SITE_PATH,
      ...(SITE_URL.port ? { ports: [Number(SITE_URL.port)] } : {}),
    }],
  }
);
